(function () {
  ...
  function levelBadge(level) {
    ...
    return `<span class="badge bg-orange-100 text-orange-700">Bronze</span>`;
  }
    ...
    mount.innerHTML = `<div class="mx-auto max-w-7xl px-4 pb-16 pt-28 sm:px-6 lg:px-8">
      ...
      <div class="grid gap-8 lg:grid-cols-[1fr_380px]">
        <section class="card p-5">
          ...
          <div class="mt-7 grid gap-4 md:grid-cols-2"><article class="rounded-3xl bg-slate-50 p-5"><h3 class="font-bold">Kelengkapan</h3><ul class="mt-3 grid gap-2 text-sm text-slate-600">${product.includes.map(item => `<li>${item}</li>`).join("")}</ul></article><article class="rounded-3xl bg-slate-50 p-5"><h3 class="font-bold">Kondisi Barang</h3><div class="mt-4 h-3 rounded-full bg-slate-200"><div class="progress-animated h-full rounded-full bg-gradient-brand" style="width:${product.condition}%"></div></div><p class="mt-2 text-sm text-slate-500">${product.condition}% sangat layak pakai</p></article></div>
          <div class="mt-5 grid gap-4 md:grid-cols-2"><article class="rounded-3xl bg-blue-50 p-5"><h3 class="font-bold text-brand-blue">Rules Box</h3><p class="mt-2 text-sm text-blue-700">${product.notes}</p></article><article class="rounded-3xl bg-teal-50 p-5"><h3 class="font-bold text-teal-700">Safety Info</h3><p class="mt-2 text-sm text-teal-700">Pembayaran aman dan tercatat. QR serah terima dipakai saat COD.</p></article></div>
