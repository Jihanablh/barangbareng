(function () {
  ...
  const componentFallbacks = {
    ...
    "#navbar-mount": `<nav id="site-navbar" class="fixed inset-x-0 top-0 z-50 border-b border-slate-100 bg-white/90 backdrop-blur-xl"><div class="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8"><button class="gradient-text text-xl font-extrabold" data-nav="home">BarangBareng</button><div class="hidden items-center gap-6 lg:flex"><button class="nav-link" data-nav="home">Beranda</button><button class="nav-link" data-nav="browse">Jelajah</button><button class="nav-link" data-scroll-target="how-it-works">Cara Kerja</button><button class="nav-link" data-scroll-target="pinjam-gratis">Pinjam Gratis</button><button class="nav-link" data-modal="upload">Sewakan Barang</button></div><button id="mobile-menu-btn" class="grid h-11 w-11 place-items-center rounded-2xl bg-white shadow-card lg:hidden"><i data-lucide="menu" class="h-5 w-5"></i></button></div></nav><aside id="mobile-drawer" class="drawer fixed inset-y-0 left-0 z-[70] w-[320px] bg-white p-5 shadow-lg lg:hidden"><button id="drawer-close" class="mb-4 rounded-full bg-slate-100 px-4 py-2 font-bold">Tutup</button><div class="grid gap-2"><button class="drawer-link" data-nav="home">Beranda</button><button class="drawer-link" data-nav="browse">Jelajah</button><button class="drawer-link" data-modal="upload">Mulai Sewakan</button></div></aside>`
  };
